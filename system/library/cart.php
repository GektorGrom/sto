...
class Cart {
	...
	private $data = array();
		...
		if (!$this->data) {
			foreach ($this->session->data['cart'] as $key => $quantity) {
				...
				if ($product_query->num_rows) {
					...
					$orig_price = $product_query->row['price'];
					$price = $orig_price;

					//bounce
					if ($this->customer->isLogged())
					{
						$bounce_prices = $this->db->query("SELECT * FROM " . DB_PREFIX . "bounce_users WHERE keysd = '" . $this->customer->getId() . "_" . $product_id . "'");
						if (isset($bounce_prices->row['valuesd'])) $bounce_prices = unserialize($bounce_prices->row['valuesd']);
						else $bounce_prices = array(array(time(),$price,0));	
					}
					else
					{
					if (isset($this->session->data['bounce']['guest_' . $product_id])) $bounce_prices = $this->session->data['bounce']['guest_' . $product_id];
					else $bounce_prices = array(array(time(),$price,0));
					}
					...
					$this->data[$key] = array(
						...
						'total'           => ($orig_price + $option_price) * $quantity - ($orig_price + $option_price) + ($price + $option_price),
  	...
  	public function add($product_id, $qty = 1, $option = array()) {
		...
		$orig_price = round($product_query->row['price']);
		$price = $orig_price;
		...
		if ($this->customer->isLogged()){
			...
			$this->db->query("UPDATE " . DB_PREFIX . "bounce_stats SET Clicks = '" . (-$i+2) . "' WHERE User = '0' AND Item = '" . $product_id . "' AND Time = '" . $bounce_prices[count($bounce_prices)-1][0] . "'");
			$this->session->data['bounce']['guest_' . $product_id] = $bounce_prices;
		}
		$this->data = array();
  	}
