  ...
  <div id="content" class="s_wrap">
    ...
    <div id="product_page" class="s_main_col">
      ...
      <div id="product" itemscope itemtype="http://data-vocabulary.org/Product">
        ...
        <?php if ($price): ?>
        <div id="product_price" class="s_price_holder s_size_4 s_label"><a id = "bounce_price">
          <?php if (!$special): ?>
...
<!-- button for paid bounce -->
        <!-- <a id="paid_bounce" class="s_button_1 s_main_color_bgr">
                <span class="s_text">Платный клик</span>
        </a>
       --></div>
        <?php endif; ?>
        <?php $tbSlot->stop(); ?>
        ...
        <div class="pluso" data-options="medium,round,line,horizontal,counter,theme=06" data-services="vkontakte,odnoklassniki,facebook,twitter" data-background="#ebebeb"></div>
            </div>
  ...
  $('#bounce_price').on('click', function(event) {
    if(buttonfalse) {return false}
      else {
        event.preventDefault()
    var spanq=$(this).find(".s_price").children("span");
    var tmp = spanq.children();
    $(this).effect('bounce', {distance:30,times:2});
    buttonfalse=true;
    spanq.fadeOut(600);

    $.ajax({
      url: 'index.php?route=product/product/getBounce',
      data: {product_id: <?php echo $product_id; ?>},
      type: 'post',
      cache: false,
      success: function(output) {
        spanq.queue(function(next){
      $(this).text(output).append(tmp).fadeIn('fast');
      next();
    });
        // KreditPrice(output);
        buttonfalse=false;
      }
      // buttonfalse=window.setTimeout((buttonfalse=false);2000)
      });
  ...
  // $('#paid_bounce').on('click', function(event) {
  //   if(buttonfalse) {return false}
  //     else {
  //       event.preventDefault()
  //    var spanq=$('#bounce_price').find(".s_price").children("span");   
  //    var tmp = spanq.children();
  //   $('#bounce_price').effect('bounce', {distance:30,times:2});
  //   buttonfalse=true;
  //   $('.s_price > span').fadeOut(600);

  //   $.ajax({
  //     url: 'index.php?route=product/product/getPaidBounce',
  //     data: {product_id: <?php echo $product_id; ?>},
  //     type: 'post',
  //     cache: false,
  //     success: function(output) {
