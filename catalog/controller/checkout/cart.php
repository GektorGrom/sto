    	...
    	if ($this->cart->hasProducts() || !empty($this->session->data['vouchers'])) {
			...
			$products = $this->cart->getProducts();
      		...
      		foreach ($products as $product) {
				...
				// Display prices total
				if (($this->config->get('config_customer_price') && $this->customer->isLogged()) || !$this->config->get('config_customer_price')) {
					$total = $this->currency->format($this->tax->calculate($product['orig_price'], $product['tax_class_id'], $this->config->get('config_tax')) * $product['quantity']-$this->tax->calculate($product['orig_price'], $product['tax_class_id'], $this->config->get('config_tax'))+$this->tax->calculate($product['price'], $product['tax_class_id'], $this->config->get('config_tax')));
				} else {
					$total = false;
				}
