	...
	public function confirm($order_id, $order_status_id, $comment = '', $notify = false) {
		...
		if ($order_info && !$order_info['order_status_id']) {
			...
			foreach ($order_product_query->rows as $order_product) {
				...
				//bounce
				$bounce_prices = $this->db->query("SELECT * FROM " . DB_PREFIX . "bounce_users WHERE keysd = '" . $this->customer->getId() . "_" . (int)$order_product['product_id'] . "'");
				$bounce_prices = unserialize($bounce_prices->row['valuesd']);
				$bounce_prices[] = $bounce_prices[count($bounce_prices)-1];
				$bounce_prices[count($bounce_prices)-2] = array(time(),"bought",0);
				 $i = 3;
				 $test = "";
				 while ($test != "bought" && $i < count($bounce_prices)-1) {
				 	$test = $bounce_prices[count($bounce_prices)-$i][1];
				 	$i++;
				 }
				 if ($test != "bought") $i += 1;
				 $this->db->query("UPDATE " . DB_PREFIX . "bounce_stats SET Clicks = '" . ($i-3) . "' WHERE User = '" . $this->customer->getId() . "' AND Item = '" . (int)$order_product['product_id'] . "' AND Time = '" . $bounce_prices[count($bounce_prices)-1][0] . "'");
			
				$this->db->query("UPDATE " . DB_PREFIX . "bounce_users SET valuesd = '" . serialize($bounce_prices) . "' WHERE keysd = '" . $this->customer->getId() . "_" . (int)$order_product['product_id'] . "'");
